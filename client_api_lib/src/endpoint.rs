...
pub trait RoutePart<Cx: ClientContext, E>: Send + Sync + 'static {
    fn apply(ep: &E, client: &ApiClient<Cx>, route: &mut RouteParts) -> Result<(), BuildError>;
}

/// PolicyPart modifie `Policy` (headers + query).
pub trait PolicyPart<Cx: ClientContext, E>: Send + Sync + 'static {
    fn apply(ep: &E, client: &ApiClient<Cx>, policy: &mut Policy) -> Result<(), BuildError>;
}

pub struct NoRoute;
impl<Cx: ClientContext, E> RoutePart<Cx, E> for NoRoute {
    fn apply(_: &E, _: &ApiClient<Cx>, _: &mut RouteParts) -> Result<(), BuildError> {
        Ok(())
    }
}
...
impl<Cx: ClientContext, E> PolicyPart<Cx, E> for NoPolicy {
    fn apply(_: &E, _: &ApiClient<Cx>, _: &mut Policy) -> Result<(), BuildError> {
        Ok(())
    }
}
...
pub struct Decoded<Dec, T>(std::marker::PhantomData<(Dec, T)>);
...
pub struct Mapped<R, M>(std::marker::PhantomData<(R, M)>);
