...
Debug sinks and runtime hooks are metadata-only observers. They receive sanitized metadata views: URLs are redacted before callback invocation, request and response headers are wrapped in a redacted header view, and they may observe statuses, retry events, rate-limit events, and endpoint metadata. `pre_send` runs after rate-limit acquisition and before raw auth transport materialization, `post_response` runs after an HTTP response is received and before response body read and endpoint decode, and `transport_error` only observes initial transport-send failures. Hooks never receive request or response body bytes, raw auth material, or a configurable runtime order.
