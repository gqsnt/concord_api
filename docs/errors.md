...
| Rate-limit acquire failure | Current advanced API returns the error from `RateLimiter::acquire`, commonly `RuntimeState` or integration-provided `ApiClientError` | Before transport | No | No | Acquire contexts are request metadata only. |
