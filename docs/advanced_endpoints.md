...
- Multipart with a one-shot stream part is one-shot unless a complete multipart factory can reconstruct every part. An all-reusable direct multipart recipe supports configured general retry and bounded authentication recovery. Each attempt builds a fresh native Reqwest form, fresh parts, and a Reqwest-owned boundary and complete `Content-Type`; materialized multipart requests are never used as replay authority.
...
- Retry policies remain available for ordinary HTTP endpoints. Built-in direct stream bodies are one-shot and are not retried unless a complete replay factory is provided. Direct multipart recipes are generally retryable when every part is reusable; multipart containing a direct one-shot stream is not. A custom `RequestEntity` must supply an explicit `PreparedBody` replay factory when it can genuinely construct a fresh terminal body for every attempt.
