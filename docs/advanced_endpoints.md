...
- `Multipart<T>` uses `MultipartBody` as a recipe for request-side `multipart/form-data` construction. It constructs native `reqwest::multipart::Form` and `Part` values only for an attempt; Reqwest owns the boundary and complete `Content-Type` value.
- Multipart with a one-shot stream part is one-shot unless a complete multipart factory can reconstruct every part. An all-reusable multipart recipe can be reconstructed for bounded authentication recovery, but remains excluded from the current general retry executor. Materialized Reqwest multipart requests are never cloneable. The private legacy `Form::into_stream` bridge is temporary; each reconstruction still gets a fresh Reqwest-owned boundary.
...
- `StreamResponse<M>` is a data-only façade over the already-limited frame-aware response body; `next_chunk()` and `write_to_file()` skip trailer frames as the explicit data-only conversion.
...
- Retry policies remain available for ordinary HTTP endpoints. Built-in stream bodies are one-shot and are not retried unless a replaying factory is provided. Direct multipart recipes are temporarily excluded from general retries even when their reusable parts allow bounded authentication reconstruction. A custom `RequestEntity` must supply an explicit `PreparedBody` replay factory when it can genuinely construct a fresh body for every attempt.
