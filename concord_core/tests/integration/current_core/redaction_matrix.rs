...
fn assert_header_matches_sentinel(
    request: &CapturedTransportRequest,
...
fn assert_query_matches_sentinel(request: &CapturedTransportRequest, key: &str, sentinel: &str) {
    let value = request
        .url
        .query_pairs()
...
fn assert_body_matches_sentinel(request: &CapturedTransportRequest, sentinel: &str) {
...
fn assert_authorization_matches_bearer_sentinel(
    request: &CapturedTransportRequest,
    sentinel: &str,
) {
    let value = request
