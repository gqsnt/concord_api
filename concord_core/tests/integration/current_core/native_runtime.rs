...
use concord_core::__private::{
    ClientPlanContext, PreparedBody, RawStreamResponse, RequestPlan, ResolvedPolicy, ResponseEntity,
};
...
async fn non_rebuildable_challenged_body_returns_original_status_without_recovery() {
    ...
    let mut client = observation_client(auth, &transport);
...
async fn non_rebuildable_challenged_stream_returns_status_after_invalidation() {
    ...
    plan.endpoint.response.format = concord_core::__private::Format::Binary;
...
async fn non_rebuildable_challenged_provider_generation_is_not_reused() {
    ...
    challenged.endpoint.policy = auth_policy(concord_core::__private::AuthPlacement::Bearer);
    ...
    let response = client
        .execute_plan::<concord_core::prelude::Text<String>>(request_plan(
            ...
            Method::GET,
            "/provider-next-call",
            auth_policy(concord_core::__private::AuthPlacement::Bearer),
            None,
        ))
        .await
