...
impl<Cx: ClientContext, T: Transport> ApiClient<Cx, T> {
    ...
    pub async fn execute_plan_websocket<Out, In, Codec>(
        &self,
        plan: RequestPlan,
    ) -> Result<crate::websocket::WebSocketClient<Out, In>, ApiClientError>
    where
        Out: Send + 'static,
        In: Send + 'static,
        Codec: crate::websocket::WebSocketCodec<Out, In>,
    {
        let RequestPlan {
            endpoint,
            mut args,
        ...
        if !matches!(endpoint.body, crate::endpoint::BodyPlan::None) {
            return Err(ApiClientError::PolicyViolation {
                ctx: ErrorContext {
                    endpoint: endpoint.meta.name,
                    method: endpoint.meta.method.clone(),
                },
                msg: "websocket endpoints do not support HTTP request bodies",
            });
        }
        let plan = crate::endpoint::RequestPlanView { endpoint, overrides };
        let ctx = ErrorContext {
        ...
        if plan.endpoint.pagination.is_some() {
            return Err(ApiClientError::PolicyViolation {
                ctx: ctx.clone(),
                msg: "websocket responses do not support pagination",
            });
        }
        if plan.endpoint.response.no_content {
            return Err(ApiClientError::PolicyViolation {
                ctx: ctx.clone(),
                msg: "websocket responses cannot use a no-content response plan",
            });
        }
        let dbg = plan.overrides.debug_level.unwrap_or_else(|| self.debug_level());
        let dbg_verbose = dbg.is_verbose();
        ...
        let auth_http = ClientAuthHttpExecutor { client: self };
        let current_attempt = checked_attempt(plan.overrides.attempt, 0, &ctx)?;
        let meta = plan
            .endpoint
            .meta
            .request_meta(current_attempt, plan.overrides.page_index);
        ...
        crate::websocket::websocketize_url(&mut built.url, ctx.clone())?;
        let url_str = built.debug_url();
        ...
        match self.transport.connect_websocket(transport_req).await {
            Ok(conn) => {
                let status = conn.status;
                let headers = conn.headers.clone();
                let rate_limit_action = if let Some(status) = status {
                    let observe_ctx = ResponseObservationCtx {
                        endpoint: conn.meta.endpoint,
                        method: &conn.meta.method,
                        url: conn.url.as_str(),
                        url_host: conn.url.host_str(),
                        attempt: conn.meta.attempt,
                        page_index: conn.meta.page_index,
                        idempotent: conn.meta.idempotent,
                        plan: &conn.rate_limit,
                        status,
                        headers: &headers,
                    };
                    ...
                    if dbg_verbose {
                        ...
                        self.debug_sink.response_headers(dbg, &headers);
                    }
                    Some(rate_limit_action)
                } else {
                    None
                };
                if let Some(status) = status {
                    if status != StatusCode::SWITCHING_PROTOCOLS {
                        return Err(ApiClientError::HttpStatus {
                            ...
                            rate_limit: rate_limit_action
                                .filter(|action| !matches!(action, RateLimitResponseAction::Continue))
                                .map(Box::new),
                        });
                    }
                }
                Ok(crate::websocket::WebSocketClient::new::<Codec>(conn))
            }
            Err(err) => {
                let hook_meta = HookMeta {
                    endpoint: built_meta.endpoint,
                    method: &built_meta.method,
                    url: &url_str,
                    attempt: built_meta.attempt,
                    page_index: built_meta.page_index,
                    idempotent: built_meta.idempotent,
                };
                self.runtime_state
                    .hooks()
        ...
        loop {
            ...
            match send_result {
                Ok(resp) => {
                    ...
                    return Ok(crate::multipart_response::MultipartStream::new(
                        resp,
                        boundary,
                        response_limit,
