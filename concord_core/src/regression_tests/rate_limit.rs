...
use crate::regression_tests::test_api::{BufferedResponse, ResponseEntity};
...
use concord_core::prelude::{ApiClientError, RateLimitObservation, RateLimitObserver};
...
impl RegressionEndpoint<TestCx> for ObservationFailureEndpoint {
...
impl RegressionReusableEndpoint<TestCx> for ObservationFailureEndpoint {
    fn plan(
        &self,
...
impl RegressionEndpoint<TestCx> for HostlessEndpoint {
    type Response = String;
...
impl RegressionReusableEndpoint<TestCx> for HostlessEndpoint {
    fn plan(
        &self,
        _ctx: &RegressionPlanContext<'_, TestCx>,
    ) -> Result<RequestPlan, ApiClientError> {
...
async fn rate_limit_observes_final_429_without_resending() -> Result<(), ApiClientError> {
    ...
    assert_eq!(sent_harness.sent_count().await, 1);
    let events = events.lock().await.clone();
...
async fn missing_host_fails_before_rate_limit_acquire() {
    ...
    assert_eq!(sent_harness.sent_count().await, 0);
    let events = events.lock().await.clone();
...
-> Result<(), ApiClientError> {
    ...
    let decoded = client
        .request(TextEndpoint {
            policy: auth_policy(crate::regression_tests::test_api::AuthPlacement::Basic),
            ..Default::default()
        })
        .response()
        .await?;
...
-> Result<(), ApiClientError> {
    ...
    assert_eq!(sent_harness.sent_count().await, 2);
    assert_error_chain_does_not_contain_any(&err, &[RESPONSE_SENTINEL]);
    Ok(())
}
...
async fn no_default_rate_limit_non_empty_plan_fails_closed() {
    ...
    policy.auth = auth_policy(crate::regression_tests::test_api::AuthPlacement::Bearer).auth;
