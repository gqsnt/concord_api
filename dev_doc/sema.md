...
Public request-shaping expressions are closed during sema. Header and query policy values, timeout expressions, route atoms, public `fmt[...]` pieces, and pagination assignment values must not resolve through auth material, secret variables, or generated implementation locals.

Route atoms split into trusted string literals and dynamic data: static path string atoms stay as raw route fragments, while dynamic path pieces and `fmt[...]` path pieces are checked as single encoded segments and reject `/`, `\`, `.` and `..`. Dynamic host pieces are checked as host labels, not as full URL hosts.
